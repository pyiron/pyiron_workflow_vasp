...
This bug could ship silently because no unit test ever fed the parser a real
OUTCAR -- every existing test either mocked the parser or used a
hand-written OUTCAR fixture too synthetic to exercise `Outcar.from_file`'s
trigger-line search logic realistically.
...
def test_parse_vasp_directory_without_vasprun_or_potcar_emits_no_warnings(tmp_path):
    ...
    files must not warn at all. Measured over a real campaign this class of
    bug produced ~2 spurious warnings per archived directory, none
...
def test_process_outcar_warns_instead_of_silently_swallowing_missing_field():
    ...
    Confirms a genuinely-missing field still degrades to np.nan (unchanged
    behaviour), but the failure is no longer invisible.
    """
    outcar = Outcar()
    outcar.from_file(str(FIXTURE_DIR / "OUTCAR"))
